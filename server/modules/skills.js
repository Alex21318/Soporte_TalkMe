...
// Apunta al archivo .env en la raiz del proyecto (server/)
    ...
    try {
        ...
        const plantillas = rows.map(row => {
            ...
            return {
                ...
                id_plantilla_categoria: row.ID_PLANTILLA_CATEGORIA,
...
// 🔹 ACTUALIZAR PLANTILLA (URL, PANTALLAS, NOMBRE, ESTADO, etc.)
router.put('/api/plantillas-whatsapp/:id', async (req, res) => {
    const { db_key } = req.query;
    const { id } = req.params;
    const { url, pantallas, nombre_plantilla, nombre, estado, estado_gupshup, id_plantilla_categoria, modificado_por, parametros } = req.body;
    ...
    try {
        ...
        if (nombre_plantilla !== undefined) {
            updates.push('NOMBRE_PLANTILLA = ?');
            params.push(nombre_plantilla);
        }
        if (nombre !== undefined) {
            updates.push('NOMBRE = ?');
            params.push(nombre);
        ...
        if (estado_gupshup !== undefined) {
            updates.push('ESTADO_GUPSHUP = ?');
            params.push(estado_gupshup);
        }
        if (id_plantilla_categoria !== undefined) {
            updates.push('ID_PLANTILLA_CATEGORIA = ?');
            params.push(id_plantilla_categoria);
        }
        if (modificado_por !== undefined) {
            updates.push('MODIFICADO_POR = ?');
            params.push(modificado_por);
        }
        ...
        if (parametros && Array.isArray(parametros)) {
            for (const param of parametros) {
                if (param.id_plantilla_parametro) {
                    const paramUpdates = [];
                    const paramParams = [];
                    
                    if (param.nombre !== undefined) {
                        paramUpdates.push('NOMBRE = ?');
                        paramParams.push(param.nombre);
                    }
                    if (param.placeholder !== undefined) {
                        paramUpdates.push('PLACEHOLDER = ?');
                        paramParams.push(param.placeholder);
                    }
                    if (param.orden !== undefined) {
                        paramUpdates.push('ORDEN = ?');
                        paramParams.push(param.orden);
                    }
                    if (modificado_por !== undefined) {
                        paramUpdates.push('MODIFICADO_POR = ?');
                        paramParams.push(modificado_por);
                    }
                    
                    if (paramUpdates.length > 0) {
                        paramParams.push(param.id_plantilla_parametro);
                        const paramQuery = `UPDATE PLANTILLA_WHATSAPP_PARAMETROS SET ${paramUpdates.join(', ')} WHERE ID_PLANTILLA_PARAMETRO = ?`;
                        await dbDestino.query(paramQuery, paramParams);
                    }
                }
            }
        }
...
router.get('/api/plantillas-categorias', async (req, res) => {
    const { db_key } = req.query;
    ...
    try {
        ...
        const [rows] = await dbDestino.query(`
            SELECT 
                ID_PLANTILLA_CATEGORIA,
                CATEGORIA,
                OPTIN,
                ETIQUETA,
                ESTADO
            FROM PLANTILLA_CATEGORIA
            WHERE ESTADO = 1
            ORDER BY CATEGORIA
        `);
...
router.get('/api/plantillas-whatsapp/:id/parametros', async (req, res) => {
    const { db_key } = req.query;
    const { id } = req.params;
    ...
    try {
        ...
        const [rows] = await dbDestino.query(`
            SELECT 
                ...
                ID_PLANTILLA,
                ID_PLANTILLA_TIPO_DATO,
                ID_PLANTILLA_EXPRESION,
                NOMBRE,
                PLACEHOLDER,
                ORDEN,
                CREADO_EL,
                ...
                MODIFICADO_EL,
                MODIFICADO_POR
            FROM PLANTILLA_WHATSAPP_PARAMETROS
            WHERE ID_PLANTILLA = ?
            ORDER BY ORDEN
        `, [id]);
        ...
        res.status(500).json({ error: 'Error al obtener parámetros', details: err.message });
    }
});

// 🔹 ACTUALIZAR PARÁMETROS DE UNA PLANTILLA
router.put('/api/plantillas-whatsapp/:id/parametros', async (req, res) => {
    const { db_key } = req.query;
    const { id } = req.params;
    const { parametros, modificado_por } = req.body;
    ...
    try {
        ...
        if (!parametros || !Array.isArray(parametros)) {
            return res.status(400).json({ error: 'parámetros requerido como array' });
        }
        ...
        for (const param of parametros) {
            if (param.id_plantilla_parametro) {
                ...
                if (updates.length > 0) {
                    params.push(param.id_plantilla_parametro);
                    const query = `UPDATE PLANTILLA_WHATSAPP_PARAMETROS SET ${updates.join(', ')} WHERE ID_PLANTILLA_PARAMETRO = ?`;
                    await dbDestino.query(query, params);
                }
            }
        }
        ...
        res.status(500).json({ error: 'Error al actualizar parámetros', details: err.message });
    }
});
...
// ==========================================================================
//  BOTS — Listado simple para selectors
// ==========================================================================
router.get('/api/bots-empresa', async (req, res) => {
    const { db_key, id_empresa, search } = req.query;
    if (!db_key || !pools[db_key]) return res.status(400).json({ error: 'db_key invalido' });

    try {
        let query = `SELECT ID_BOT, DESCRIPCION AS NOMBRE_BOT, DESCRIPCION, ID_EMPRESA, ESTADO FROM BOT WHERE ESTADO = 1`;
        const params = [];
        if (id_empresa) {
        ...
        query += ` ORDER BY DESCRIPCION ASC LIMIT 500`;
        const [rows] = await pools[db_key].query(query, params);
        res.json(rows);
    } catch (err) {
...
// ==========================================================================
//  HORARIO_BOT — CRUD de horarios para bots
// ==========================================================================
router.get('/api/horarios-bot', async (req, res) => {
    const { db_key, id_bot, id_empresa } = req.query;
    if (!db_key || !pools[db_key]) return res.status(400).json({ error: 'db_key invalido' });

    try {
        let query = `SELECT h.ID_HORARIO_BOT, h.ID_BOT, h.DESDE, h.HASTA, h.DIAS, h.CREADO_EL, h.CREADO_POR, h.MODIFICADO_EL, h.MODIFICADO_POR, b.DESCRIPCION AS NOMBRE_BOT
                     FROM HORARIO_BOT h
                     JOIN BOT b ON h.ID_BOT = b.ID_BOT
                     WHERE 1=1`;
        const params = [];
        if (id_bot) {
            query += ` AND h.ID_BOT = ?`;
        ...
        query += ` ORDER BY h.ID_HORARIO_BOT DESC`;
        const [rows] = await pools[db_key].query(query, params);
        res.json(rows);
    } catch (err) {
        console.error('[/api/horarios-bot GET]', err);
        res.status(500).json({ error: err.message });
    }
});

router.post('/api/horarios-bot', authMiddleware, async (req, res) => {
    const { db_key, id_bot, desde, hasta, dias } = req.body;
    const creado_por = req.user?.usuario || 'sistema';
    if (!db_key || !pools[db_key]) return res.status(400).json({ error: 'db_key invalido' });
    if (!id_bot || !desde || !hasta || !dias) return res.status(400).json({ error: 'id_bot, desde, hasta y dias son requeridos' });

    try {
        const [result] = await pools[db_key].query(
            `INSERT INTO HORARIO_BOT (ID_BOT, DESDE, HASTA, DIAS, CREADO_POR) VALUES (?, ?, ?, ?, ?)`,
            [id_bot, desde, hasta, dias, creado_por]
        );
        res.json({ ok: true, id_horario_bot: result.insertId });
    } catch (err) {
        console.error('[/api/horarios-bot POST]', err);
        res.status(500).json({ error: err.message });
    }
});

router.put('/api/horarios-bot/:id', authMiddleware, async (req, res) => {
    const { id } = req.params;
    const { db_key, desde, hasta, dias } = req.body;
    const modificado_por = req.user?.usuario || 'sistema';
    if (!db_key || !pools[db_key]) return res.status(400).json({ error: 'db_key invalido' });

    try {
        await pools[db_key].query(
            `UPDATE HORARIO_BOT SET DESDE = ?, HASTA = ?, DIAS = ?, MODIFICADO_POR = ? WHERE ID_HORARIO_BOT = ?`,
            [desde, hasta, dias, modificado_por, id]
        );
        res.json({ ok: true });
    } catch (err) {
        console.error('[/api/horarios-bot PUT]', err);
        res.status(500).json({ error: err.message });
    }
});

router.delete('/api/horarios-bot/:id', authMiddleware, async (req, res) => {
    const { id } = req.params;
    const { db_key } = req.query;
    if (!db_key || !pools[db_key]) return res.status(400).json({ error: 'db_key invalido' });
    ...
    try {
        await pools[db_key].query(
            `DELETE FROM HORARIO_BOT WHERE ID_HORARIO_BOT = ?`,
            [id]
        );
        res.json({ ok: true });
    } catch (err) {
        console.error('[/api/horarios-bot DELETE]', err);
        res.status(500).json({ error: err.message });
    }
});
