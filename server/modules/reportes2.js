...
router.post('/api/reportes/notasrapidas', async (req, res) => {
    ...
    try {
        ...
        if (id_nota_rapida && id_nota_rapida !== '') {
            whereConditions.push('A.ID_NOTAS_RAPIDAS = ?');
...
// ==========================================================================
//   REPORTE RECONTACTOS PROGRAMADOS
// ==========================================================================
router.post('/api/reportes/recontactos-programados', async (req, res) => {
    const { db_key, id_empresa, fecha_inicio, fecha_fin, skills, id_bots, id_redes_sociales, creado_por, estado } = req.body;
    ...
    try {
        ...
        const fechaFinStr = fechaFinAjustada.toISOString().split('T')[0] + ' 00:00:00';

        const skillsFilter = skills && skills.length > 0 ? skills.join(',') : null;
        const skillsCondition = skillsFilter ? `AND CP.ID_SKILL IN (${skillsFilter})` : '';
        
        // Para bots, necesitamos filtrar por ID_BOT_REDES, pero el frontend envía ID_BOT
        // Hacemos un JOIN con BOT_REDES para obtener los ID_BOT_REDES correspondientes
        const botsArr = Array.isArray(id_bots) ? id_bots.map(n => parseInt(n, 10)).filter(Number.isFinite) : [];
        let botsCondition = '';
        let botJoin = '';
        if (botsArr.length > 0) {
            botJoin = `INNER JOIN (SELECT DISTINCT ID_BOT_REDES FROM BOT_REDES WHERE ID_BOT IN (${botsArr.join(',')})) BR_FILTRO ON BR_FILTRO.ID_BOT_REDES = CP.ID_BOT_REDES`;
        }
        
        const redesArr = Array.isArray(id_redes_sociales) ? id_redes_sociales.map(n => parseInt(n, 10)).filter(Number.isFinite) : [];
        const redesCondition = redesArr.length > 0 ? `AND RS.ID_RED_SOCIAL IN (${redesArr.join(',')})` : '';
        
        const operadorCondition = creado_por ? `AND CP.CREADO_POR = ?` : '';
        ...
        const estadoCondition = estado && estadosMap[estado] !== undefined ? `AND CP.ESTADO = ${estadosMap[estado]}` : '';

        const query = `
            SELECT 
                CP.ID_RRSS_EXTERNO AS Identificador,
                CP.NOMBRE_CLIENTE AS Cliente,
                CP.CREADO_POR AS 'Creado Por',
                DATE_SUB(CP.CREADO_EL, INTERVAL 6 HOUR) AS 'Fecha Creacion',
                DATE_SUB(CP.FECHA_PROGRAMADA, INTERVAL 6 HOUR) AS 'Fecha Programada',
                B.DESCRIPCION AS Canal,
                RS.NOMBRE AS 'Red Social',
                CP.NOMBRE_SKILL AS Skill,
                TC.NOMBRE_TIPO AS 'Tipo Cliente',
                CASE CP.TIPO_CONTACTO
                    WHEN 0 THEN 'Manual'
                    WHEN 1 THEN 'Automático'
                    ELSE 'Desconocido'
                ...
                CASE CP.ESTADO
                    ...
                    WHEN 3 THEN 'Vencido'
                    WHEN 4 THEN 'Pospuesto'
                END AS Estado,
                CP.COMENTARIO AS 'Comentario',
                DATE_SUB(CP.FECHA_EJECUCION, INTERVAL 6 HOUR) AS 'Fecha Ejecucion',
                UA.NOMBRE_USUARIO AS 'Usuario Asignado',
                UC.NOMBRE_USUARIO AS 'Usuario Cancelacion'
            FROM CONTACTO_PROGRAMADO CP
            ${botJoin}
            LEFT JOIN USUARIOS UA ON UA.ID_USUARIO = CP.ID_USUARIO_ASIGNADO
            LEFT JOIN USUARIOS UC ON UC.ID_USUARIO = CP.ID_USUARIO_CANCELACION
            LEFT JOIN BOT_REDES BR ON BR.ID_BOT_REDES = CP.ID_BOT_REDES
            LEFT JOIN BOT B ON B.ID_BOT = BR.ID_BOT
            LEFT JOIN REDES_SOCIALES RS ON RS.ID_RED_SOCIAL = BR.ID_RED_SOCIAL
            LEFT JOIN TIPO_CLIENTE TC ON TC.ID_TIPO = CP.ID_TIPO_CLIENTE
            WHERE CP.FECHA_PROGRAMADA >= ?
        ...
        const params = [fecha_inicio, fechaFinStr];
        if (operadorCondition) params.push(creado_por);

        const [rows] = await pool.query(query, params);
        const data = rows.map(limpiarFila);
        
        res.json(data);
        
    } catch (error) {
        console.error('Error en /api/reportes/recontactos-programados:', error);
        res.status(500).json({ error: 'Error al ejecutar reporte de recontactos programados: ' + error.message });
    }
});
