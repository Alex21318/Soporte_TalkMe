...
async function programarCronTareas() {
    ...
    try {
        ...
        for (const [hora, grupo] of Object.entries(porHora)) {
            ...
            const nextRun = job.nextDate ? job.nextDate() : 'N/A';
            console.log(`[TareasScheduler] 📅 Próximo disparo: ${nextRun} (ahora=${new Date().toISOString()})`);
        }
    } catch (e) {
...
async function ejecutarTarea(tarea) {
    ...
    try {
        ...
        const resultado = await new Promise((resolve, reject) => {
            ...
            req.on('timeout', () => {
                req.destroy();
                reject(new Error('Timeout: el servidor no respondió en 30s'));
            });
            req.write(body);
            req.end();
        });
...
// GET /api/tareas/ultimo-log?tipo=&db_key=  — última ejecución solamente
router.get('/api/tareas/ultimo-log', async (req, res) => {
    try {
        const { tipo, db_key } = req.query;
        if (!tipo || !db_key) return res.status(400).json({ error: 'Faltan tipo y db_key' });
        const [rows] = await db.query(`
...
// ── Catch-up: ejecutar tareas que debieron correr hoy pero no lo hicieron ──
async function validarEjecucionesPendientes() {
    try {
        const tareas = await leerTareas();
        const activas = tareas.filter(t => t.activo && t.hora);
        if (!activas.length) {
            console.log('[TareasScheduler] Sin tareas activas para validar catch-up');
            return;
        }

        const ahoraUTC = new Date();
        const ahoraGT  = new Date(ahoraUTC.getTime() - 6 * 60 * 60 * 1000);
        const hoyGT    = ahoraGT.toISOString().split('T')[0]; // YYYY-MM-DD
        const horaActualGT = ahoraGT.toTimeString().slice(0, 5); // HH:MM

        console.log(`[TareasScheduler] 🔍 Validando catch-up · hoy=${hoyGT} · hora=${horaActualGT} GT`);

        for (const t of activas) {
            const [logs] = await db.query(`
                SELECT DATE_SUB(EJECUTADO_EL, INTERVAL 8 HOUR) AS ejecutado_gt
                FROM SCHEDULER_TAREAS_LOG
                WHERE TIPO = ? AND DB_KEY = ?
                ORDER BY EJECUTADO_EL DESC
                LIMIT 1
            `, [t.TIPO, t.DB_KEY]);

            let ejecutadoHoy = false;
            if (logs.length > 0 && logs[0].ejecutado_gt) {
                const fechaLog = new Date(logs[0].ejecutado_gt);
                const fechaLogGT = fechaLog.toISOString().split('T')[0];
                ejecutadoHoy = fechaLogGT === hoyGT;
            }

            if (ejecutadoHoy) {
                console.log(`[TareasScheduler] ✓ ${t.NOMBRE || t.TIPO} (${t.DB_KEY}) ya ejecutada hoy`);
                continue;
            }

            if (horaActualGT >= t.hora) {
                console.log(`[TareasScheduler] ⚠️ ${t.NOMBRE || t.TIPO} (${t.DB_KEY}) NO ejecutada hoy y la hora ${t.hora} GT ya pasó. Ejecutando catch-up...`);
                await ejecutarTarea(t);
            } else {
                console.log(`[TareasScheduler] ⏳ ${t.NOMBRE || t.TIPO} (${t.DB_KEY}) NO ejecutada hoy, pero aún no es la hora (${t.hora} GT)`);
            }
        }
    } catch (e) {
        console.error('[TareasScheduler] Error validando ejecuciones pendientes:', e.message);
    }
}

// Iniciar cron de tareas al cargar el modulo
programarCronTareas();
validarEjecucionesPendientes();
