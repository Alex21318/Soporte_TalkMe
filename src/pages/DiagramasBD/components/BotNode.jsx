...
export default function BotNode({ id, data, selected }) {
  ...
  // Nodo LOGO
  if (data.type === 'LOGO') {
    const imgSrc = data.imageUrl.startsWith('/assets/') || data.imageUrl.startsWith('data:') ? data.imageUrl : `https://wsrv.nl/?url=${encodeURIComponent(data.imageUrl)}`;
    return (
      <div className="flex items-center justify-center p-4 bg-transparent pointer-events-none opacity-90">
        <img src={imgSrc} alt="Logo" crossOrigin={data.imageUrl.startsWith('/assets/') ? undefined : "anonymous"} className="max-w-[280px] max-h-[120px] object-contain drop-shadow-xl" />
      </div>
    );
  }
    ...
    <div ref={nodeRef} className={`bg-white flex flex-col ${selected ? 'ring-4 ring-blue-100' : ''} border border-slate-200 ${hasJumps ? 'rounded-t-xl rounded-b-md' : 'rounded-xl'} shadow-lg w-[340px] transition-all duration-200 overflow-visible ${isNotVisible ? 'opacity-95 border-dashed' : ''} border-t-4 ${topBorderColor} z-30 relative`}>
      ...
      {!isMenuNode && (
        <div className="flex flex-col flex-grow z-20 relative">
          ...
          {showImage && (
            <div className="p-2 border-b border-slate-50 bg-white"><div className="w-full h-40 bg-slate-50 rounded-lg flex items-center justify-center p-1 overflow-hidden shadow-inner border border-slate-100"><img src={data.imagen.startsWith('/assets/') || data.imagen.startsWith('data:') ? data.imagen : `https://wsrv.nl/?url=${encodeURIComponent(data.imagen)}`} alt="Media" crossOrigin={data.imagen.startsWith('/assets/') ? undefined : "anonymous"} loading="lazy" className="max-w-full max-h-full object-contain" onError={(e) => { e.target.style.display = 'none'; }} /></div></div>
          )}
          {showUrlFile && (
