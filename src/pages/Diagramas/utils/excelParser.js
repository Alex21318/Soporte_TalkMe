    ...
    reader.onload = (e) => {
      try {
        ...
        generalData.forEach((item) => {
          ...
          // --- Rutas individuales (con división de sourceValues múltiples) ---
          const jumpsTo = [];
          if (idEncadenar) {
            jumpsTo.push({
              type: 'chain',
          ...
          nodeRoutings.forEach(routing => {
            ...
            const rawSourceValue = valorKey ? String(routing[valorKey]).trim() : '';
            
            // 🔴 DIVIDIR SI CONTIENE COMAS (múltiples opciones en una misma línea)
            ...
            // Crear una entrada por cada valor individual
